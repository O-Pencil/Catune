...
- **i18n（双语化）**：zh / en 双语。UI 走 `useT()` / `tr(locale, key)`；引擎的 `postureLabel` / `ruleFallback` 在 `setLocale()` 时立即 emit 新 locale 文案；端侧教练 `coachPrompt.ts`（指令 + 姿态信号）、`actionTag.ts`（模型输出 `[动作:xxx]` / `[Action:xxx]` 标签解析）、`assess/parse.ts`（VL 评估 system/user prompt + JSON fallback summary）、`assess/preset.ts`（3 套预置结果）、`mnn/deviceProfile.ts`（设备推荐 reason/details）、`BenchmarkScreen`（DEFAULT_PROMPT + PREVIEW_STATUS 字段）都按 locale 切换。字典平铺 key→string，en/zh 对齐由 `src/ui/i18n/__tests__/coverage.test.ts` 守门（键数一致、双向覆盖、值非空）。缺 key 时 dev 模式 console.warn（`missing-key.test.ts` 守门）。
