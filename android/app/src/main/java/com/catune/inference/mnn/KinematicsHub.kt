 ...
 *
 * [WHO] 提供 `object KinematicsHub`、`enum Posture(NORMAL/SLUMPED/TECH_NECK/LEFT_LEAN/OFFLINE)`、`data class State(neckPitch, lumbarRoll, posture, score, abnormalDurationMinutes)`、`update(neck, lumbar)`、`setOffline()`
 * [FROM] 依赖 `kotlinx.coroutines.flow.MutableStateFlow`
 * [TO] 被 `KinematicsModule` RN 桥接订阅；被 `SpineBluetoothManager.processIncomingData()` 写入
 * [HERE] android/app/src/main/java/com/catune/inference/mnn/KinematicsHub.kt · 姿态状态枢纽（RN/BLE 共享）
 */
