/**
 * @file DeskScreen.tsx
 * @description 书桌/仪表盘屏（= web DeskPage 的 RN 还原）：问候头 + 设备状态卡 + 脊柱可视化(3 节点) + 节点角度卡 + 建议。浅色 Haptic。
 *   脊柱可视化先用纯 RN View（线 + 3 个节点点）近似 web 的 SVG+桌面图；后续可上 react-native-svg + 桌面图与脉冲动画。
 *
 * [WHO] 导出 `DeskScreen`
...
function Node({label, color, top}: {label: string; color: string; top: number}): React.JSX.Element {
  return (
    <View style={[styles.nodeSlot, {top}]}>
  ...
  return (
    <Card style={styles.spineCard}>
      ...
      <View style={styles.spineArea}>
        ...
        <Node label="C7" color={cNeck} top={20} />
        <Node label="T12" color={cThor} top={110} />
        <Node label="L5" color={cLum} top={200} />
      </View>
    </Card>
  );
}
...
const styles = StyleSheet.create({
  ...
  spineArea: {flex: 1, marginTop: 40, position: 'relative'},
  spineLine: {position: 'absolute', top: 24, bottom: 24, width: 3, borderRadius: 2, backgroundColor: theme.colors.border, alignSelf: 'center'},
  nodeSlot: {position: 'absolute', left: 0, right: 0, alignItems: 'center'},
  nodeInner: {alignItems: 'center', justifyContent: 'center'},
  dotOuter: {width: 18, height: 18, borderRadius: 9, borderWidth: 2.5, alignItems: 'center', justifyContent: 'center', backgroundColor: theme.colors.surface},
  ...
  tagText: {fontSize: 10, fontWeight: theme.font.weightBold, color: theme.colors.textPrimary},
