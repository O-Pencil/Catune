 ...
 * [FROM] 依赖 `react`、`react-native`、`../../mnn/modelCatalog`、`../../mnn/modelDownloadService`
...
import {releaseMnnModel} from '../../mnn/nativeDebugClient';
import {
  DeviceTier,
  getDeviceProfile,
  getTierLabel,
...
export function ModelDownloadCard({onModelsChanged}: Props): React.JSX.Element {
  const t = useT();
  const {locale} = useLocale();
  const docDir: string | null = getModelDocumentDirectory();
          ...
          onPress: async () => {
            ...
            if (activeId === selectedId) {
              ...
              if (remaining.length > 0) {
                ...
                await clearActiveModelId(docDir);
              }
              await releaseMnnModel();
            }
